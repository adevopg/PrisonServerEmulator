...
def dump(a,b,label=""):
    ...
    for ins in md.disasm(tdata[off:off+(b-a)], a):
        ...
        for g,nm in [(0x5ec5d0,"classArr"),(0x5ec5d4,"arr1"),(0x5ec5d8,"arr2"),
                     (0x5ec5ee,"?"),(0x5bd448,"screen"),(0x5eda68,"prisonTree")]:
            if f"0x{g:x}" in ins.op_str: ann+=f"  ; {nm}"
        print(f"  {ins.address:08x}: {ins.mnemonic:8s} {ins.op_str}{ann}")
# CLASSINFO UI handler (event 0x48e -> 0x4a4620): builds [0x5ec5d0]
dump(0x4a4620, 0x4a4760, "CLASSINFO UI 0x4a4620 (builds classArr [0x5ec5d0])")
